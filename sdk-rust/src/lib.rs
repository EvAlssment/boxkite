...
//! `docs/API.md` in the [boxkite repository](https://github.com/HarshitKmr10/boxkite)
...
//! connections/webhooks. Deliberately **not** included in this pass (see
//! `sdk-rust/README.md`'s "Scope" section for the full disclosure): a
...
//! ecosystem convention), the control-plane's secrets CRUD and
//! filesystem-snapshot routes (real endpoints, but not wrapped by
//! `sdk-python`/`sdk-js` either as of this writing). Unlike that last group,
//! this crate is genuinely **behind** its two reference SDKs -- not merely
//! mirroring them -- on account/usage introspection, auth-flow helpers
//! (password reset/verify/refresh/logout), the `http_request`
//! secrets-broker proxy, network-ingress preview URLs, and the per-account
//! command allowlist; `sdk-python`/`sdk-js` wrap all of these today and
