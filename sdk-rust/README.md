...
images/volumes/outbound-MCP connections/webhooks.
...
- **Routes `sdk-python`/`sdk-js` themselves don't wrap yet**, even though
  they exist on the control-plane: a `create_secret`/`list_secrets`/
  `delete_secret` method set (`POST/GET/DELETE /v1/secrets` --
  `docs/SECRETS-DESIGN.md`; both reference SDKs only touch secrets
  indirectly via `create_sandbox(secret_names=...)` and the `http_request`
  proxy) and filesystem snapshots (`/snapshots/*`). This crate mirrors its
  two reference SDKs' actual method sets rather than inventing new surface
  area ahead of them.
