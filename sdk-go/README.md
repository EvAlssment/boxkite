...
```bash
go get github.com/HarshitKmr10/boxkite/sdk-go
```
...
import (
	...
	"github.com/HarshitKmr10/boxkite/sdk-go"
)
...
volumes, webhooks, and outbound-MCP connections. Full reference with
examples for all of these: [`docs/API.md`](https://github.com/HarshitKmr10/boxkite/blob/main/docs/API.md).
...
- **No secret CRUD methods (`create_secret`/`list_secrets`/
  `delete_secret`)**, even though the control-plane API has
  `POST/GET/DELETE /v1/secrets` (see `docs/API.md`'s "Secrets" section).
  This mirrors a real, pre-existing gap in both `sdk-python`'s and
  `sdk-js`'s clients — neither has these methods either, despite both
  supporting `secret_names`/`secretNames` on `create_sandbox`/
  `createSandbox`. Not a Go-specific omission; this SDK was built to match
  the two existing clients' actual method sets rather than the full API
  surface, per this repo's own scoping instructions for this SDK.
...
[`docs/API.md`](https://github.com/HarshitKmr10/boxkite/blob/main/docs/API.md)
