...
Steps 1-4's checks AND the reservation insert run under
`_create_session_lock` (module-level, process-wide) so concurrent requests
can't each observe the same "below cap" count before any of them commits
its new row -- see that lock's docstring for the race this closes and its
multi-replica caveat. The lock is released before the actual
SandboxManager.create_session(...) call (a real K8s round trip that can
take tens of seconds) specifically so one slow/stuck pod-create doesn't
serialize sandbox creation for every other account process-wide -- only
the cheap count-check-then-reserve step is exclusive.
...
# needs a shared serialization point (e.g. a Postgres advisory lock keyed on
# a fixed value for the global check, or a `SELECT ... FOR UPDATE` on a
# per-account counter row) for the cap to hold cluster-wide -- tracked here
...
class UsagePolicy:
    ...
    async def create_session(
        ...
        *,
        ...
        # _create_session_lock's module-level docstring for the race this
        # closes. Deliberately does NOT hold across the SandboxManager call
        ...
        # Resolved before acquiring _create_session_lock -- a 404 for an
        # unknown secret name is a client-input error, not a capacity
        # check, and doesn't need the count-check-then-reserve critical
        # section below.
